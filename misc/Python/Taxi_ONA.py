...
NAR.AddInput("*babblingops=6")
actions = {"^south" : 0, "^north" : 1, "^east" : 2, "^west" : 3, "^pickup" : 4, "^drop_off" : 5} #gym encoding

for i, x in enumerate(actions):
    NAR.AddInput("*setopname " + str(i+1) + " " + x) #NAR encoding *setopname 1 ^left *setopname 2 ^down  *setopname 3 ^right *setopname 4 ^up
goal = "G"
  
#-1 per step unless other reward is triggered. +20 delivering passenger. -10 executing “pickup” and “drop-off” actions illegally. done True if reach the goal or time reset!
#Setup environment:
...
for i in range(0, 100000):
    ...
    if reward > -2: #-1 per step unless other reward is triggered. +20 delivering passenger
        successes += 1
    if reward > 0:
        NAR.AddInput(goal + ". :|:")
        successes_episode += 1
...
data.to_csv('Taxi_v1_ONA_'+str(seed_num)+'.csv', index = False)
